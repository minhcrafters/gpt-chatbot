...
def message(self, update, context):
    ...
    for turn in turns[from_index:]:
        ...
        for user_message in turn["user_messages"]:
            prompt += (
                clean_text(user_message) + self.generation_pipeline.tokenizer.eos_token
            )
        for bot_message in turn["bot_messages"]:
            prompt += (
                clean_text(bot_message) + self.generation_pipeline.tokenizer.eos_token
            )
...
class TelegramBot:
    ...
    def __init__(self, **kwargs):
        ...
        dp.add_handler(
            MessageHandler(MessageFilter.text, self_decorator(self, message))
        )
